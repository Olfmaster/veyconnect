...
export default function LegalLayout({ eyebrow, title, intro, children }) {
  return (
    <article className="relative bg-base text-fg-muted pt-32 md:pt-40 pb-24 px-6 md:px-10 border-b border-line/5">
      <div className="max-w-3xl mx-auto">
        <p className="label-mono text-[#9162a4] mb-6">
          / {eyebrow}
        </p>
        <h1 className="text-4xl md:text-6xl font-semibold tracking-[-0.03em] leading-[0.95] mb-8 text-fg">
          {title}
        </h1>
        {intro && <p className="text-lg text-fg-muted leading-relaxed mb-12">{intro}</p>}
...
export function Section({ heading, children }) {
  return (
    <section>
      <h2 className="text-xl md:text-2xl font-semibold tracking-tight text-fg mb-3">
        {heading}
      </h2>
      <div className="space-y-3 text-base text-fg-muted">{children}</div>
    </section>
  );
}
